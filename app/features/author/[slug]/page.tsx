...
import type { Work, AuthorFeatureWork } from "@/types";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const feature = await getAuthorFeatureBySlug(slug);

  if (!feature) {
    return { title: "作家特集が見つかりません" };
  }

  const year = new Date().getFullYear();
  // layout.tsx の template: "%s | DJ-ADB" が自動付与される
  const title = `【${year}年最新】${feature.author_name}特集 おすすめ同人コミック・CG厳選${feature.works.length}作品レビュー`;
  const headlineText = feature.headline ? `${feature.headline}。` : "";
  const description = `${feature.author_name}の人気同人コミック・CG${feature.works.length}作品を厳選レビュー特集。${headlineText}${feature.description || ""}FANZAで評価の高い${feature.author_name}作品を毎日更新。`.slice(0, 160);

  return {
    title,
    description,
    alternates: { canonical: `/features/author/${slug}/` },
    openGraph: {
      title,
      description,
...
export default async function AuthorFeatureDetailPage({ params }: Props) {
  const { slug } = await params;
  const [feature, allFeatures, allWorks] = await Promise.all([
    getAuthorFeatureBySlug(slug),
    getAuthorFeatures(),
    getWorks(),
  ]);
  ...
  // この作家の全作品を取得
  const authorAllWorks = await getWorksByAuthorName(feature.author_name);

  // セール中の作品（おすすめと重複しないもの）
  const saleWorks = authorAllWorks
    .filter((w) =>
  ...
  const year = new Date().getFullYear();
  const articleHeadline = `【${year}年最新】${feature.author_name}特集 おすすめ同人コミック・CG${feature.works.length}選`;
  const articleDescription =
    (feature.headline || feature.description)
      ? `${feature.author_name}の人気作品${feature.works.length}選を厳選。${feature.headline || feature.description}`
      : `${feature.author_name}の人気同人コミック・CG${feature.works.length}作品をDJ-ADB編集部が厳選レビュー。`;
  const articleUrl = `https://dj-adb.com/features/author/${slug}/`;
      ...
      <main className="mx-auto max-w-3xl px-4 py-4">
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <nav className="text-sm text-muted-foreground">
            ...
            <span className="mx-2">/</span>
            <span className="text-foreground">{feature.author_name}</span>
          </nav>
        ...
        <div className="mb-6 mt-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">{feature.author_name}</h1>
          ...
          <div className="p-4 bg-gradient-to-r from-pink-500/10 to-purple-500/10 rounded-lg border border-pink-500/20">
            ...
            <p className="text-base font-bold text-foreground">
              {feature.headline || `${feature.author_name}ファン必見の厳選${feature.works.length}作品`}
            </p>
            <p className="text-sm text-muted-foreground mt-1">
              {feature.description || `${feature.author_name}の作品から迷ったらこれを選べ。`}
            </p>
        ...
        {saleWorks.length > 0 && (
          <section className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-foreground">{feature.author_name}のセール中作品</h2>
              <Badge variant="sale" className="text-xs">
                {saleWorks.length}作品
              </Badge>
            </div>
        ...
        {newWorks.length > 0 && (
          <section className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-foreground">🆕 {feature.author_name}の最新作</h2>
              <Badge variant="secondary" className="text-xs">
                {newWorks.length}作品
              </Badge>
            </div>
        ...
        <div className="mt-8 mb-10">
          <Link href={`/authors/${encodeURIComponent(feature.author_name)}`}>
            <Card className="overflow-hidden border-2 border-pink-500 hover:border-pink-400 transition-all bg-gradient-to-r from-pink-500/10 to-purple-500/5">
              <div className="flex items-center justify-between p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-base font-bold text-foreground">
                      {feature.author_name}の全{authorAllWorks.length}作品を見る
                    </p>
                    <p className="text-sm text-muted-foreground">
                      作品を一覧表示
                    </p>
        ...
        {otherFeatures.length > 0 && (
          <section className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-foreground">他の人気作家特集</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {otherFeatures.map((other) => (
                <Link key={other.slug} href={`/features/author/${other.slug}`}>
                  <Card className="overflow-hidden border border-border hover:border-pink-500/50 transition-all">
                    <div className="relative aspect-[16/9] overflow-hidden bg-muted">
                      <img
                        src={other.thumbnail_url || "https://placehold.co/200x112/f4f4f5/71717a?text=Author"}
                        alt={other.author_name}
                        loading="lazy"
                        className="h-full w-full object-cover"
                      />
                    </div>
                    <div className="p-3">
                      <p className="text-sm font-bold text-foreground truncate">{other.author_name}</p>
