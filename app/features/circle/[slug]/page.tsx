...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `【${year}年最新】${feature.circle_name}特集 おすすめ同人コミック・CG厳選${feature.works.length}作品レビュー`;
  const headlineText = feature.headline ? `${feature.headline}。` : "";
  const description = `${feature.circle_name}の人気同人コミック・CG${feature.works.length}作品を厳選レビュー特集。${headlineText}${feature.description || ""}FANZAで評価の高い${feature.circle_name}作品を毎日更新。`.slice(0, 160);
...
export default async function CircleFeatureDetailPage({ params }: Props) {
  ...
  const year = new Date().getFullYear();
  const articleHeadline = `【${year}年最新】${feature.circle_name}特集 おすすめ同人コミック・CG${feature.works.length}選`;
  const articleDescription =
    (feature.headline || feature.description)
      ? `${feature.circle_name}の人気作品${feature.works.length}選を厳選。${feature.headline || feature.description}`
      : `${feature.circle_name}の人気同人コミック・CG${feature.works.length}作品をDJ-ADB編集部が厳選レビュー。`;
  const articleUrl = `https://dj-adb.com/features/circle/${slug}/`;
