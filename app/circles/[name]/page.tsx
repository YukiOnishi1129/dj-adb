...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // 上位ジャンルを抽出（メタディスクリプションのキーワード密度向上用）
  ...
  const title = `${saleBadge}【${year}年最新】サークル「${decodedName}」の同人コミック・CGおすすめ${works.length}選｜代表作・新作レビュー | DJ-ADB`;
  ...
  const description = `サークル「${decodedName}」の同人コミック・CG${works.length}作品を厳選レビュー。${genreText}${ratingText}${saleText}FANZAで人気の${decodedName}の作品の評価・あらすじ・抜きどころを毎日更新。`.slice(0, 160);
...
export default async function CircleDetailPage({ params }: Props) {
  ...
  const circlePageUrl = `https://dj-adb.com/circles/${name}/`;
        ...
        <Card className="mb-6 border-border">
          <CardContent className="p-5">
            ...
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              サークル<span className="font-semibold text-foreground">「{decodedName}」</span>の同人コミック・CG作品
